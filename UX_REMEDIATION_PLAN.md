...
| L-10 | Verse panel Word-study rows look clickable, aren't (Claude-5) | ⛔ **DO NOT BUILD — did not reproduce, and the proposed fix would break a deliberate design.** Driven live signed-out (dev, John 3:16 → Word study): 25 rows render, each row IS a working accordion, and every Strong's chip IS already a link to `/word/G####`. Clicking the first row (Οὕτως, which has no Strong's number) expands to `ADVERB — No dictionary entry linked (lemma οὕτω(ς))` — an honest empty state, not a dead click. The proposed exit test ("row click navigates") would REPLACE the accordion with navigation, and `study-panel.tsx:568-570` states why that is wrong on purpose: the chip is a link and a SIBLING of the toggle, never nested, because *"a link in a button is invalid and one tap cannot honestly do two things."* Building this ticket would undo a considered decision to satisfy a finding that does not reproduce. **Needs re-observation** naming the verse, the row, and what was expected — possibly a prod-only lexicon-asset failure (see CH-10), which would be a different bug with a different fix. | — |
