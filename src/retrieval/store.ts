...
// one query — no N+1.
// `createNeonStore` was removed 2026-08-02. Its only consumer was retrieval.integration.test.ts,
// which went with the dead `retrieve.ts` — knip surfaced it as an orphan the moment that test
// was deleted, which is the tool doing its job. `createPgStore` below is the live one, used by
// src/ingest/ingest-embeddings.ts and src/teacher/run.ts.
