...
// no full-graph re-walk (which is where Phase A's 12–14s came from).
//
// HISTORY OF THIS NUMBER — it tracks corpus size, so re-sweep after major growth:
//   64  (2026-07-14, v3 sweep): smallest that reliably filled 50/50 on THAT corpus, ~270ms.
//   200 (2026-08-24, ADR-118 remedy): the P4.n programme grew the corpus past ef=64's reach —
//        W-PN20 measured the pool filling 5/20 and 8/20 on two proper-noun queries (the two
//        0-voice misses behind the 17/20 gate failure, reproduced three times). Swept per the
//        pre-registration (docs/evidence/adr118-ef-lever/): pn20 HIT@2 17/20 -> 18/20 (bar
//        cleared), full v4 every category passes (several improved), controls clean, wall-clock
//        parity (1503ms vs 1493ms incl. connection). Owner-ordered remedy, bar unchanged.
// Re-tune via the eval's --ef sweep (space-separated: `--ef 400`, not `--ef=400`).
