...
describe.skipIf(SKIP)('Topic plan end-to-end (owner-seeded fixture, real handlers, real DB)', () => {
  ...
  afterAll(async () => {
    // Reverse order; sections cascade topical_entries, but be explicit —
    // a partial teardown must not strand a PUBLISHED qa source (Gate B).
    if (planId) {
      await deletePlanRoute(new NextRequest('http://localhost'), { params: Promise.resolve({ id: planId }) }).catch(() => {});
    }
    if (sourceId && ownerUrl) {
      await ownerQuery(`DELETE FROM topical_entries WHERE section_id = $1`, [sectionId]).catch(() => {});
      await ownerQuery(`DELETE FROM sections WHERE source_id = $1`, [sourceId]).catch(() => {});
      await ownerQuery(`DELETE FROM sources WHERE id = $1`, [sourceId]).catch(() => {});
    }
  }, 60_000);
