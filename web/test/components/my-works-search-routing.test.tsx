...
//
// The fix is an AND-gate: send `{ ref }` only when BOTH the shape regex AND `parseRef` agree.
...
//
// THE REQUEST IS A POST, NOT A GET. The search route is state-changing (a paid embedding + an
// audit row), so it sits behind the CSRF Content-Type floor and the component sends the routing
// decision as a JSON BODY, `{ ref }` or `{ q }`. The assertions below read that body; the routing
// property they pin is unchanged from the query-string form this test was first written against.
//
// SEED: restore the old routing (`const payload = looksLikeRef ? { ref: q } : { q }`, no
//   `parsed.ok` gate) and the quadrant-A rows go RED — "sermon 1" is sent as `{ ref: 'sermon 1' }`
//   instead of `{ q }`. The quadrant-B and quadrant-D rows stay GREEN, proving the AND-gate is
...
/** The JSON bodies the component POSTed to the search route, in order. */
let searchBodies: Array<{ ref?: unknown; q?: unknown }>;
  ...
  vi.stubGlobal(
    ...
    vi.fn(async (url: string, init?: RequestInit) => {
      ...
      if (u.includes('/api/user-corpus/search')) {
        searchBodies.push(JSON.parse(String(init?.body ?? '{}')) as { ref?: unknown; q?: unknown });
...
async function submitQuery(query: string): Promise<void> {
  ...
  await waitFor(() => {
    expect(searchBodies.length, 'a /api/user-corpus/search request was issued').toBeGreaterThan(0);
  });
}
...
describe('My Works search routing — the looksLikeRef ∧ parseRef AND-gate', () => {
  ...
  //   B (true, true): parser-confirmed passage. Must STILL send { ref }, carrying the NORMALIZED
  ...
  it.each(cases)('routes %j to { %s } (expected=%j)', async (query, param, expected) => {
    ...
    if (param === 'q') {
      expect(body.ref, 'must NOT route to the presence endpoint').toBeUndefined();
      expect(body.q, 'must carry the typed query to fused search').toBe(query);
    } else {
      expect(body.q, 'must NOT carry a fused-search query').toBeUndefined();
      expect(body.ref, 'must carry the normalized reference').toBe(expected);
    }
  });
});
