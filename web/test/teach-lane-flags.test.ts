...
// (or skips) have already happened by that point (teach.ts constructs all three
...
describe('teach(): caller-controlled lane flags', () => {
  ...
  it('fires all three lanes when lanes is omitted (unchanged default behavior)', async () => {
  ...
  it('skips every lane when every flag is false — the exegetical retrieval is never gated by it', async () => {
    await teach('What is grace?', { lanes: { sermons: false, theology: false, songVerse: false } });
    expect(retrieveSermonLane).not.toHaveBeenCalled();
