...
const findWorkOrdinalsForVerseId = vi.hoisted(() => vi.fn());

// Taxonomy stays REAL — only the two query functions and the verse-anchored ordinal lookup are
// stubbed. `findWorkOrdinalsForVerseId` lives in `@/lib/work` and runs SQL against Neon; stubbing it
...
// to findWorkOrdinalsForVerseId for `scripture:jhn/3` and `scripture:john/3` (alias → canonical).
...
beforeEach(() => {
  ...
  findWorkOrdinalsForVerseId.mockReset();
...
describe('library [catalog] F-158: add-to-desk lands near the open Scripture passage', () => {
  it('canonical scripture:jhn/3 carries :ordinal (UX_FIX_VERIFICATION.md:116 baseline)', async () => {
    findWorkOrdinalsForVerseId.mockResolvedValue(new Map([['adam-clarke', 8075]]));
    ...
    expect(findWorkOrdinalsForVerseId).toHaveBeenCalledWith([{ id: WORK.id, slug: 'adam-clarke' }], JOHN_3_1);
  });
  ...
  it('aliased scripture:john/3 now RESOLVES and carries the same :ordinal (regression for F-158)', async () => {
    ...
    // misses, findWorkOrdinalsForVerseId is never called, and the href is bare `work:adam-clarke`.
    findWorkOrdinalsForVerseId.mockResolvedValue(new Map([['adam-clarke', 8075]]));
    ...
    // The page called findWorkOrdinalsForVerseId with the SAME (work, verseId) pair as the canonical
    ...
    // Both inputs drive findWorkOrdinalsForVerseId -> 8075, so the resulting work pane matches.
  ...
  it('hyphenated alias scripture:1-john/1 resolves (normalizeBookInput rescues hyphens)', async () => {
    ...
    findWorkOrdinalsForVerseId.mockResolvedValue(new Map([['adam-clarke', 42]]));
    ...
    expect(findWorkOrdinalsForVerseId).toHaveBeenCalledWith([{ id: WORK.id, slug: 'adam-clarke' }], 62_001_001);
  });
  ...
  async () => {
    ...
    expect(findWorkOrdinalsForVerseId).not.toHaveBeenCalled();
  });

  it('null ordinal from findWorkOrdinalsForVerseId falls back to bare work:<slug> but still carries the desk',
  async () => {
    ...
    findWorkOrdinalsForVerseId.mockResolvedValue(new Map([['adam-clarke', null]]));
    const href = await addHrefFor('scripture:john/3');
    expect(href).not.toBeNull();
    const panes = paneValues(href!);
    expect(panes).toContain('scripture:john/3');
    expect(panes).toContain('work:adam-clarke');
    expect(panes.some((p) => /^work:adam-clarke:\d+$/.test(p)), 'no work:adam-clarke:<ordinal>').toBe(false);
    expect(findWorkOrdinalsForVerseId).toHaveBeenCalledOnce();
  });
