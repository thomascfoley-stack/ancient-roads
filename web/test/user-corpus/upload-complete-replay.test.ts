...
import { blobPathname } from '@/lib/user-corpus/blob';
...
// `blobPathname` stays REAL: the D11 heal re-homes the row's bytes onto it, and the guard under
// test compares the incoming pathname against that value. A reimplemented copy here would assert
// the test's own arithmetic instead of the route's.
...
vi.mock('@/lib/user-corpus/documents', async (importOriginal) => {
  // isHealable/healPlan stay REAL — they choose which dedupe outcome runs, so faking them would
  // let this suite pass against a branch the route does not take.
  ...
  return {
    isHealable: actual.isHealable,
    healPlan: actual.healPlan,
    requeueForRetry: async (_userId: string, id: string, opts: { resetAttempts?: boolean } = {}) => {
      const d = docs.get(id);
      if (!d) return false;
      docs.set(id, { ...d, status: 'queued', attempts: opts.resetAttempts ? 0 : d.attempts });
      return true;
    },
    DuplicateDocument,
...
describe('upload-complete — replay and the dedupe-delete guard', () => {
  ...
  it('an existing row whose blobUrl is null still deletes the incoming orphan', async () => {
    ...
    // The row's live pathname (after the heal, below) !== orphan pathname → the orphan is cleaned up.
    expect(BYTES.has(orphan)).toBe(false);

    // ASSERTION REVISED ON PORT (this branch predates D11). It read
    // `expect(docs.get('doc-null-blob')!.blobUrl).toBe(null)` — "the existing row keeps its (null)
    // blob_url unchanged". D11 landed on main afterwards and deliberately changed exactly that:
    // a blobUrl-null row is HEALABLE, so this re-upload is now the repair gesture — healPlan says
    // 'store-and-requeue', the bytes are stored at the canonical `blobPathname`, and blob_url is
    // set to it. Asserting null now would pin the pre-D11 no-op, so the assertion follows the
    // behaviour that legitimately changed. What it must NOT lose is the guard's own property, and
    // that is asserted harder here than before: the row's live pathname is a DIFFERENT object from
    // the deleted orphan, and it still holds bytes.
    const healed = docs.get('doc-null-blob')!;
    expect(healed.blobUrl).toBe(blobPathname(USER.id, 'doc-null-blob'));
    expect(healed.blobUrl).not.toBe(orphan);
    expect(BYTES.has(healed.blobUrl!), 'the healed row points at bytes that exist').toBe(true);
  });

  it('a healable row whose blobUrl IS the replayed pathname keeps its blob (the guard, on the heal path)', async () => {
    // The replay case again, one branch over: a FAILED document whose blob_url is the very
    // pathname being replayed. Pre-guard, healOrDuplicate re-queued the row and then deleted the
    // bytes it had just told the drain to read — the corrupt-report loop, with the row now marked
    // 'queued' so the drain is guaranteed to hit it.
    docs.set(
      'doc-failed',
      makeDoc({
        id: 'doc-failed',
        checksum: await realChecksumOf(CONTENT),
        blobUrl: PATHNAME,
        status: 'failed',
        attempts: 1,
      }),
    ...
    expect(res.status).toBe(200);
    const body = (await res.json()) as { duplicateOf: string; healed: boolean };
    expect(body.duplicateOf).toBe('doc-failed');
    expect(body.healed).toBe(true);

    // THE FIX: the row was re-queued for another attempt AND its bytes are still there to read.
    expect(docs.get('doc-failed')!.status).toBe('queued');
    expect(BYTES.has(PATHNAME), 'the re-queued row still has its bytes').toBe(true);
  });
});
