...
export function scanReferenceSpans(text: string, opts: ParseOptions = {}): ScannedSpan[] {
  ...
  for (const m of text.matchAll(DIGIT_ATTACHED_SCAN_RE)) {
    // Merge 2026-08-24: main widened `consider` to take the book word and whether the span
    // carried an ordinal (the SCAN_RE corroboration work). A digit-attached form IS an ordinal —
    // "1Cor" is "1 Cor" — so it passes true, not false.
    consider(`${m[1]} ${m[2]} ${m[3]}`.replace(/\s+/g, ' ').trim(), m.index!, m.index! + m[0].length, m[2]!, true);
  }
