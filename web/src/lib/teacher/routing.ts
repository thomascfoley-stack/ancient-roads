...
// `spurgeon-talks-to-farmers` was added 2026-08-02 by owner decision ("add it so all sermons are
// live") and REMOVED 2026-08-11 by owner decision ("kill it — we will upload it later"): the work
// was published with 298 sections but embeddings were never generated, so it was shelf-readable
// and retrieval-invisible. It is now serve:false in the manifest (the re-upload will carry
// vectors). Its non-authorial back matter suppression (ordinals 299-300) stays on record for
// that re-ingest.
