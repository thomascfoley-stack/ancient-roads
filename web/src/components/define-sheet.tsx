  ...
  return (
    ...
    >
      <div className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-t-2xl bg-paper shadow-deep animate-slide-up dark:bg-stone-900 dark:text-stone-300">
        <div className="sticky top-0 z-10 flex items-start justify-between border-b border-stone-100 bg-white/95 px-5 py-4 backdrop-blur-sm dark:border-stone-800 dark:bg-stone-900/95">
          <div>
            <p className="font-display text-xl text-stone-900 dark:text-stone-100">“{english}”</p>
            <p className="mt-0.5 text-sm text-stone-500 dark:text-stone-400">{reference}</p>
          </div>
          <button
            onClick={onClose}
            className="rounded-full p-1.5 text-stone-500 hover:bg-stone-100 hover:text-stone-600 dark:text-stone-400"
            aria-label="Close"
          >
              ...
              <ul className="space-y-2">
                {matches.map((m) => (
                  <li key={`${m.word.s}-${m.index}`}>
                    <button
                      onClick={() => onPick(m)}
                      className="flex w-full items-baseline justify-between gap-3 rounded-xl bg-white px-4 py-3 text-left shadow-paper transition-colors ease-gentle hover:bg-amber-50 dark:bg-stone-800 dark:shadow-none dark:hover:bg-stone-700"
                    >
                      <span>
