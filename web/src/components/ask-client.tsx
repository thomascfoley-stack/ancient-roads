...
export function AskClient({ initialThread }: { initialThread?: InitialThread } = {}) {
  ...
  const ask = useCallback(async (raw: string, replaceId?: number) => {
    const q = raw.trim();
    if (!q || busy) return;
    track({ name: 'question_asked', is_followup: replaceId !== undefined });
        ...
        {turns.length === 0 && (
          <div className="pb-8">
            ...
            {/* THE INVITATION INTO HISTORY (order 2026-08-20-historians-study-entrance): the
                ...
                Deliberately countless ("history's own witnesses", not "28 works"): a hardcoded
                corpus count here is the watchlist's hand-maintained-fact shape, stale on the next
                ingest. And NOT "the church's historians": the corpus leads with Josephus, a
                first-century Jewish historian, so a churchmen-only claim mischaracterises it
                (deep-audit domain finding 4). The shelf itself is deliberately neutral ("Historians"). */}
            <Link
            ...
            >
              ...
              <span className="mt-1 block text-sm leading-relaxed text-stone-500 dark:text-stone-400">
                Herod, the church at Ephesus, the fall of Jerusalem — history&rsquo;s own witnesses, opened to the exact page.
              </span>
