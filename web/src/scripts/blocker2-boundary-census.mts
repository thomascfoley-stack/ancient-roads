...
const q1 = (await sql.query(`
  ...
  FROM embeddings WHERE user_id IS NULL AND ${PROSE_TYPE_SQL}`)) as any[];
const r1 = q1[0];
...
const bySlug = ((await sql.query(`
  ...
  WHERE e.user_id IS NULL AND ${PROSE_E}`)) as any[])[0];
const byAuthor = ((await sql.query(`
...
const unmatched = (await sql.query(`
  ...
  GROUP BY 1 ORDER BY rows DESC LIMIT 15`)) as any[];
...
const q3 = ((await sql.query(`
  SELECT count(*)::int AS legal_rows, count(DISTINCT metadata->>'author')::int AS legal_authors
  FROM embeddings WHERE user_id IS NULL AND ${PROSE_TYPE_SQL} AND ${LEGAL_CORPUS_FILTER}`)) as any[])[0];
...
const q4 = ((await sql.query(`
  ...
  WHERE e.user_id IS NULL AND ${PROSE_E}
    AND s.status='published' AND s.license IN ${ALLOWED}`)) as any[])[0];
...
const gains = (await sql.query(`
  ...
  GROUP BY 1,2 ORDER BY rows DESC LIMIT 20`)) as any[];
const gainTot = ((await sql.query(`
  SELECT count(*)::int AS rows FROM embeddings e
  WHERE e.user_id IS NULL AND ${PROSE_E} AND ${DATA_PRED} AND NOT (${LEGAL_E})`)) as any[])[0];
const losses = (await sql.query(`
  SELECT e.metadata->>'author' AS author, e.metadata->>'work' AS work, count(*)::int AS rows
  FROM embeddings e
  WHERE e.user_id IS NULL AND ${PROSE_E} AND ${LEGAL_E} AND NOT (${DATA_PRED})
  GROUP BY 1,2 ORDER BY rows DESC LIMIT 20`)) as any[];
const lossTot = ((await sql.query(`
  SELECT count(*)::int AS rows FROM embeddings e
  WHERE e.user_id IS NULL AND ${PROSE_E} AND ${LEGAL_E} AND NOT (${DATA_PRED})`)) as any[])[0];
...
const book = async (lo: number, hi: number, pred: string) => {
  const r = ((await sql.query(`
    ...
    FROM embeddings e WHERE e.user_id IS NULL AND ${PROSE_E} AND ${pred}
      AND (metadata->>'verseId')::bigint >= ${lo} AND (metadata->>'verseId')::bigint < ${hi}`)) as any[])[0];
...
const drift = (await sql.query(`
  ...
  GROUP BY 1,2 ORDER BY rows DESC LIMIT 20`)) as any[];
const driftTot = ((await sql.query(`
  SELECT count(*)::int AS rows FROM embeddings e LEFT JOIN sources s ON ${KEY}
  WHERE e.user_id IS NULL AND ${PROSE_E} AND ${LEGAL_E}
    AND (s.slug IS NULL OR s.status <> 'published')`)) as any[])[0];
