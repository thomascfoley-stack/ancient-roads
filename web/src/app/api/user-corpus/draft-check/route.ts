...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  // The limiter used to run immediately after the auth guard — ahead of the CSRF floor and the
  // body validation above — so a wrong-Content-Type, empty, or oversize request (the very inputs
  // the floor and body guards exist to reject) returned 400/413 having already bumped
  // corpus-search:min/day. Every sibling route carrying both guards puts the floor ahead of the
  // limiter (ask, ask/stream, upload-url, upload-complete, history/search); this route alone had
  // them inverted — commit f1d36b72 placed the new floor after the already-present limiter call.
  // The shared corpus-search:* bucket is unchanged; only the ordering is fixed. Still before
  // draftCheck(), so any check that runs is metered.
