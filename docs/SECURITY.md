...
- **next 16.2.12 → 16.3.5** (`web/package.json`): clears GHSA-p293-qw3h-jr36 (RCE on Windows-hosted servers, >=16.0.0 <16.3.3) and GHSA-2xp9-vwfh-vxw4 (RCE in Image Optimization via AVIF, >=16.0.0 <16.3.3). Both critical. Patch-level bump within the existing 16.x line.
- **sharp → 0.35.4** (root `pnpm.overrides`): clears GHSA-rgj7-g3m4-5g8c (libheif vulnerabilities GHSA-g89c-p67h-r497 and GHSA-2jg2-4ch7-h545, <0.35.4). High. One patch above previous 0.35.3.
