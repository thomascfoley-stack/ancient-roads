...
| `NEXT_PUBLIC_POSTHOG_KEY` / `NEXT_PUBLIC_POSTHOG_HOST` | PostHog analytics + error tracking (owner ruling O-2). Client init in `web/src/instrumentation-client.ts`; the SDK **dials PostHog's own origin directly** — the same-origin `/ingest` rewrite was deliberately REMOVED by owner ruling 2026-08-18 (it tunnelled a third party inside `'self'` with our cookies attached). **CSP is therefore load-bearing:** `script-src` AND `connect-src` in `web/next.config.ts` must both name the PostHog hosts. The previous wording here ("beacons are same-origin … so CSP is untouched") was stale from that ruling and is the exact belief under which analytics went 100% dark in production for an unknown period — `script-src` omitted the assets host while `connect-src` named it. Pinned since 2026-08-24 by `web/test/posthog-wiring.test.ts` | Vercel | **Total no-op** — `instrumentation-client.ts` skips `posthog.init` without the key; dev/CI need nothing. HOST defaults to `https://us.i.posthog.com` |
