...
| `NEXT_PUBLIC_POSTHOG_KEY` / `NEXT_PUBLIC_POSTHOG_HOST` | PostHog analytics + error tracking (owner ruling O-2). Client init in `web/src/instrumentation-client.ts`; beacons are same-origin via the `/ingest` rewrite in `web/next.config.ts`, so CSP is untouched | Vercel | **Total no-op** — `instrumentation-client.ts` skips `posthog.init` without the key; dev/CI need nothing. HOST defaults to `https://us.i.posthog.com` |
