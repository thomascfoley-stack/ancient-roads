...
- **Lexicon/dictionary serving UX.** ISBE (8,928), Easton (3,933), Nave (4,870), Smith (4,362), BDB (11,845) are ingested STAGED (never served). How to surface a topic-keyed reference work — a separate reference pane vs. blending into the exegetical pool — is a product design decision. They are NOT wired to any serving surface; the reference bridge sets `publish:false` unconditionally.
...
- **018/019 drop the live serving index before rebuilding it** — on prod this opens a starvation/scan window (the ef=40 fallback that killed migration 009). For Part C: build the replacement under a NEW name (`…_v5`) with `CREATE INDEX CONCURRENTLY` FIRST, then `DROP` the old — zero window. The dev `rebuild-register-indexes.ts` drop-then-create is fine (dev has no traffic). **Part C runbook updated.**
- **Duplicate migration number 020** (`020_embeddings_source_type_registers.sql` + `020_sources_status_ingesting.sql`). Both are idempotent; the Part C runbook lists them explicitly in dependency order. Not renamed (both already applied on dev; rename risks confusion) — the runbook is the source of order.
