...
4. **Logos-style, in our skin.** Our colors/type/spacing, not a clone. ~~Tap-verse → commentaries is untouched.~~ **SUPERSEDED 2026-08-02 by ADR-047** (`docs/DECISIONS.md`): the handle moved to the verse NUMBER specifically, so the verse text is free for double-click-to-select. Owner-ruled, not re-opened by an agent.
...
- ~~**Do not change tap-a-verse → commentaries.** The existing Bible reading path stays byte-behavior-identical.~~ **SUPERSEDED 2026-08-02 — see ADR-047.** The old whole-verse handler broke double-click-to-select-a-word (the sheet's scrim ate the second click); the owner ruled to move the handle to the verse number. This is no longer a live boundary — do not treat future edits to `verse-display.tsx`'s click target as a STOP on this basis alone.
