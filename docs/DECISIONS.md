...
**Consequences.** Two grammars → one. The rail loses ~10 glyphs and three date stamps at the owner's row count. `GroupItem.icon` becomes optional. `dayStamp`/`WEEKDAYS`/`accentDot` are deleted. Rows lose 20px of label width inside an open panel. The pr1c test's third leg is re-pointed from "links to `/prayers`" to "not rendered, storage untouched" (C1: re-pointed under a ruling, not edited to pass; the leg was watched go red first).
