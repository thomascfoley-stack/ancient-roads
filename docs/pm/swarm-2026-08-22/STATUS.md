...
| W-REGDURABLE | 1c | **DONE-UNVERIFIED (tooling); dev flip MOOT** — preconditions no longer hold: zero register='sermon'/'theology' rows remain and all 9 published sermon/theology works already fully served on dev. Tool `scripts/register-flip-batched.mjs` (dry-run default, dev-guarded, DB-is-the-checkpoint) with a thorough resume red-proof (SIGTERM mid-run, no-double-apply, batch atomicity, prod guards). Prod flip (~4h resumable) → packet B6 | `swarm/w-regdurable-flips` @ a81a95b |
