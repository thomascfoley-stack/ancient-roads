...
**2026-08-17 addendum (N1/N3/N4/N5/N6 slices).** Done 39 -> 44. §5b grew 2 -> 6. FIVE are closed — N1/N3/N5/N6 fixed, N4 closed as a recorded decision to keep it quarantined (see its row). **N2 is now CLOSED too** (owner-directed 2026-08-17): it was held back as a design change because it moves the composer on every mobile view, and that is exactly what it does — 52px reclaimed, measured before and after. **CORRECTED 2026-08-18 by the pre-deploy audit.** This sentence claimed §1 had "exactly 44 data rows" and called it re-measured. It has **53**, and it had 53 when the claim was written (`git show 667e571` confirms) — the 8 rows restored on 2026-08-17 plus one more were appended without bumping the count, and my counting regex missed them. The number now says 53. §4's 22 was and is correct. The lesson is the instrument, not the number: an ad-hoc regex over a markdown table is not a measurement, and calling it one is how a wrong count acquires authority.
...
| N2 | `/ask` reserved the mobile tab bar twice — composer floated 71px above it | `73293f1` (mask corrected by `e1aa459`/`e736b9b`) |
