...
| # | Item | Note |
|---|---|---|
| UX-1 | **The Bible cannot be reached on the desk** | **CLOSED — fixed 2026-08-02 in `5760eec`, recorded 2026-08-23 (W-UX1).** This row first said the pane model could not hold Scripture and needed a design decision. Wrong — `lib/desk.ts` already has `kind:'scripture'` beside `kind:'work'` and `/desk` renders both, so a Bible pane opens today by URL. The remaining **picker gap** (`+` routed to `/library?desk=…`, works only) closed the same day: BookPicker gained a pick mode and the desk's add rail carries an "Add a Bible chapter" button beside `+` (empty state: "Open the Bible"), appending a Scripture pane in place, no navigation. Guards: `web/test/components/desk-empty-cta-adds-scripture.test.tsx`, `desk-cap-notice.test.tsx`. Browser re-verified 2026-08-23: `docs/evidence/swarm-2026-08-22/w-ux1/` |
