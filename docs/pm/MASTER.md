...
| # | Gate | Status |
|---|---|---|
| D1 | Fork merge — `worktree-corpus-cdn-build` + `main` both grew the serving lists | **DONE 2026-08-15**, [PR #90](https://github.com/thomascfoley-stack/ancient-roads/pull/90) @ `08aca18`, audit green. Three checks red on the union, all real: **`gill-song` has been out of production's `idx_commentary_fts_legal` since 113 applied (2026-08-14)** — 113 was rebuilt from a `routing.ts` that never carried it, while the other branch had it deployed and serving since 2026-08-12 (`509d690`). Neither branch was wrong alone; the union is what could see it. Also: the `publish-flip-toolchain` serve:false fixture moved for the **third** time (thayers published by close-out #4 → `josephus-works`), and `ObsFields` widened for the per-attempt timing arrays |
| D2 | ⚑ Migration **115** on prod | **OPEN — and it gates D1's deploy.** Not a formality: prod's index is 113-era, this branch's query predicate includes `gill-song`, and a query predicate BROADER than a partial index's predicate means the planner **cannot use that index at all**. Applied to dev only (ledger `sha256 1840d90f0d00…`, `valid=true`, predicate confirmed live) |
