...
| A5 | ⚑ Prod instrument run — G10 stops being permanently skipped | **UNBLOCKED by A4.** The `--cohort=published` instrument run is one read-only command, ready now. **G10 fork discharge stays open** (needs a Neon fork; branch creation forbidden, ADR-043 wanted it BEFORE the flip — that departure is now a fact, record it in the A6 go). [readiness](orders/2026-08-01-a3-a6-readiness.md) |
| A6 | ⚑ Deploy A — the irreversible one | **DONE 2026-08-02.** `dpl_Dcd5Gy93UGqrP1z7Hq9U7cHwUbhg`, READY, aliased to `ancientpaths.app`, from `main` @ `0768fcd`. 170.8 MB uploaded, all seven `deploy.sh` steps, receipt at `evidence/deploys/deploy-0768fcd.txt`. **The first attempt failed** (`98124b2`): the remote build died on `Module not found: '../../../src/ingest/forbidden-provenance.mjs'` — `vercel --prod` uploads `web/` ALONE, so the specifier resolved in the checkout, in CI, in `npm run audit` and in a local `next build`, and did not exist in the deployment. A gate that runs in a different tree shape than production; `test/invariants/web-upload-root.test.ts` now catches it statically. The two-clone problem and the Vercel link, which this row named as blockers, were both already closed — see M24. |
