...
| A6 | ⚑ Deploy A — the irreversible one | Blocked on A5. **The two-clone problem is CLOSED** (deep audit M24): the board cited DEPLOY_PREFLIGHT §9 as a blocker for its own irreversible gate after it had stopped being true. Re-measured 2026-08-02 in `~/Projects/ancient-roads-git` — all six served dirs present, HEAD current, `served-assets.mjs` present, `predeploy-gate.ts` exits 0 at `DEPLOYING=1`. One clone, code and corpus together. The **Vercel project link** was the other half and is now pinned by `deploy.sh` (`EXPECT_PROJECT_ID`/`EXPECT_ORG_ID`). **Preflight now measured, not guessed** — [DEPLOY_PREFLIGHT.md](../DEPLOY_PREFLIGHT.md). The build was **BROKEN at `d1576fe`** (fixed `c1e359d`, and `next build` is in CI since `19798ec`); `predeploy-gate` passes on the real corpus. `concordance`/`lexicon`/`original` were ABSENT; **`b9ad463` restored all three and the [census](../evidence/post-a1-2026-08-01/concordance-census.md) confirms FIVE of the six served dirs are byte-exact against `corpus-backup-2026-07-28`.** The sixth, `devotional`, is not in that release at all and cannot be compared against it — it is tracked in git and restores from any clean clone (corrected 2026-08-02; "all six" was in this row, DEPLOY_PREFLIGHT §3 and its checklist item 5). Rollback ids established — [RECOVERY.md](../RECOVERY.md) §2 |
