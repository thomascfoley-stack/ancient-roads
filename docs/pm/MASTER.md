...
Last verified: 2026-09-07 · **live on `ancientpaths.app`: `5fdb8f46`** (receipt `dpl_14iyWtqAUW4pUShLh8Q6PXj4CP6o`, 2026-09-07T08:58:58Z, `alias_serves == deployment_id`, `state: live`) — **THE UNION**: the 51 `detail/*` PRs + the My Works rename (ADR-123) + the #4 false-confidence fixes, shipped together after `main` and `redesign/ask` were found to have DIVERGED with **neither containing the other**. **Every line of shipped CODE on `main` is live** — the merge-to-main gap that every header on this board has carried since 2026-08-18 is CLOSED. (`main` then moves ahead by docs-only commits, this record included, which is the normal steady state and NOT a reopened gap. The distinction is the point: check `git diff --stat origin/main <live-sha> -- web/src src` — if that is empty, the gap is closed no matter how far the shas differ.) Verified independently of the deploy script: the served CSS on the live alias contains `html.reader-dark`, added in this union and absent from the previous build, with a nonsense-class negative control returning 0. Owner-executed; the agent session's permission classifier refused `./deploy.sh`, and the first attempt was correctly BLOCKED by the ancestry gate because the deploying worktree sat one commit behind `main` — the gate did its job. Re-measure rather than read.
