...
Last verified: 2026-08-31 · **live on `ancientpaths.app`: `602bd9e`** (receipt `dpl_23XTyox8H3YN56A4w8t22LiGLwwb`, 2026-08-31T14:54Z) — the pre-open pass: fallback span strip, stale-GET misdirected-DELETE fix, envInt fail-loud on 18 limiter vars, perHour completion of the H3 loosening, `search:global:day` fleet ceiling, deploy.sh trap red-proofs + harness network isolation. Branch `fix/ux-overnight-sweep` now CONTAINS `origin/main` (ancestry merge `602bd9e`); the merge-to-main gap is inverted — main is behind the branch. Re-measure rather than read.
