...
Last verified: 2026-09-13 · **live on `ancientpaths.app`: `3dc76f20`** (receipt `dpl_DZ8TL4PYZcmjNh4L8TSHjW6ugTAc`, 2026-09-13T17:46:21Z, `alias_serves == deployment_id`, `state: live`) — **next 16.3.5 + the 17-fix Detail batch (`#317` + `#318`)**: clears GHSA-p293-qw3h-jr36 and GHSA-2xp9-vwfh-vxw4 (next, critical, fixed ≥16.3.3) and GHSA-rgj7-g3m4-5g8c (sharp, high, ≥0.35.4), and the 17 #318 fixes (#244 #245 #251 #252 #253 #288 #290 #292 #294 #295 #296 #300 #301 #302 #307 #309 #312); plus the `root-params.d.ts` import that 16.3.5's build writes to `web/next-env.d.ts`. Verified independently of the deploy script by fingerprinting the `/gate` chunk: #301's `advanced_disable_feature_flags:!0` is present (1 chunk; 0 in the previous deploy `dpl_2S3qFezZaS32CuWScxsiKcr3mwYc`), so the check is not vacuous. `main` (the merge `c6652f80`) and `live` differ only by docs-only commits above the live sha; the code-diff predicate `git diff --stat origin/main <live-sha> -- web/src src` is empty. Owner-executed — the agent session's permission classifier refuses `./deploy.sh`. Re-measure rather than read: that predicate being empty is what closes the gap, however far the shas differ.
Prior: `6dcd674f` (receipt `dpl_2S3qFezZaS32CuWScxsiKcr3mwYc`, 2026-09-07T10:28:35Z) — **ADR-124**: one rule for rail row marks (the section glyph moves to the header; only My studies' per-study colour dot survives on a row), prayer rows lose their date stamps, and the pre-N4 custom sections are HIDDEN with their `localStorage` key deliberately untouched (the prayer carry-forward's only recovery source; possible teams/classroom feature later).
Prior: `5fdb8f46` (receipt `dpl_14iyWtqAUW4pUShLh8Q6PXj4CP6o`, 2026-09-07T08:58:58Z) — THE UNION: the 51 `detail/*` PRs + the My Works rename (ADR-123) + the #4 false-confidence fixes, shipped together after `main` and `redesign/ask` were found DIVERGED with neither containing the other; the merge-to-main gap carried on this board since 2026-08-18 was closed there.
Prior: `d323fff3` (receipt `dpl_uJpXkfgECCkQxwp2DRggg4J8UW2a`, 2026-09-07T04:47Z, `state: live` — the script's own identity check ran this time) — Sidebar C (ADR-122), the four UX sweeps (Stop stops the spend; `/search` loading; `/studies` signed-out; four small lies; Daily Office bounded; error voice + shared skeleton; per-row remove on Saved; `/home` day-sharded 367×; four dialogs with real semantics; 21 targets to 44px; a nav CTA), `main`'s hidden `db-invariants` red fixed with a guard, and the 24 deep-audit fixes (WORKLOG 2026-09-07). **Both halves of that sentence went false within the hour and are corrected here:** PR #235 **MERGED** 2026-09-07T05:05:13Z at `df6783da`, so the gap it named is closed; and `redesign/ask` stopped containing `origin/main` the moment the `detail/*` backlog landed on main. The two then **DIVERGED** — main took 60 commits (the 51 `detail/*` PRs), `redesign/ask` took 6 (the My Works rename ADR-123, the #4 false-confidence fixes, the AGENTS ground rules) — and **neither contained the other**, which no header on this board would have told you. Re-united by merge at `22087530` (one overlapping file, `my-works.tsx`, auto-merged; no conflicts). **This is the third time a `Last verified` line has been read as measurement after going stale — measure it (`git merge-base --is-ancestor origin/main HEAD`), never read it.**
Prior: `d6e85f3` (receipt `dpl_4ztNuAtoYHkTz1tfnjz9Wph7ofQZ`, READY 2026-09-07T02:33Z) — the /ask redesign (ADR-121); its identity check did not run (CLI poll timeout), verified by hand.
Prior: `602bd9e` (receipt `dpl_23XTyox8H3YN56A4w8t22LiGLwwb`, 2026-08-31T14:54Z) — the pre-open pass: fallback span strip, stale-GET misdirected-DELETE fix, envInt fail-loud on 18 limiter vars, perHour completion of the H3 loosening, `search:global:day` fleet ceiling, deploy.sh trap red-proofs + harness network isolation. Branch `fix/ux-overnight-sweep` now CONTAINS `origin/main` (ancestry merge `602bd9e`); the merge-to-main gap is inverted — main is behind the branch. Re-measure rather than read.
Prior: `0e69854` (receipt `dpl_EjbQXfcz5cKGWDxwyfQd5tfVXnhh`, 2026-08-30T17:56Z) — the UX overnight sweep + direct-to-Blob upload slice + RLS proof + api-error revert: 13 UX fixes, F-134 cap fix, deploy.sh fixed path, presigned-URL upload bypassing the platform body cap, RLS structural and behavioural proof recorded, api-error.ts framework-free again.
