...
| B0a | K re-validation on a fresh held-out set | **UNBLOCKED 2026-08-03 by ADR-103, not yet run.** Was NOT DONE because the frozen marker regex requires quote-then-reference (Spurgeon's CCEL house style), so Wesley/Edwards/Whitefield gave eligible n=0 against a floor of 20. ADR-103's metric needs no epigraph, so the set is now buildable. Scheduled as §4 of the Slice 1 build plan: derive K on set 1, validate on a disjoint set 2, report the exclusion rate. Harness still deliberately NOT widened — the parser work is demoted, not deleted. [prior result](../evidence/slice0-k-revalidation/RESULT.md) |
...
| B4 | Translation decision — shingle against the user's translation, or all. Moved the headline 17 points | **RULED 2026-08-03 — ADR-100.** Option A, per-**document** detection, no user setting in Slice 1. Three things the [paper](../SLICE1_TRANSLATION_DECISION.md) left open are closed by the ADR: the bar is **end-to-end channel recall with detection running, not detector top-1 accuracy** (the 18 translations are not equidistant — kjv↔akjv costs ~0, kjv↔web costs ~17 points); detection resolves to a **family** and unions within it when the top two are close, with a below-floor fallback **recorded** in `user_section_anchors.confidence` rather than applied silently; families are **derived from measured 6-gram overlap, never typed**. Carries an UNVERIFIED flag — the within-family-overlap premise is reasoning, not measurement, and is the first thing step 3 measures. Overturned by measuring precision at K=3 under all-translation shingling |
| B5 | Slice 1 — prose/sermons end-to-end + the tradition-gap join | **IN PROGRESS.** B1/B3 closed; B4 ruled (ADR-100); B2 proceeds on `bge-large` per the order. Step 1 of 7 done — migration `100_user_corpus.sql` applied to `lane-b-uploader`, RLS proven two-account over `app_runtime` (28 legs, watched red). Step 2 (upload/parse/status) next |
