...
Last verified: 2026-09-07 · **live on `ancientpaths.app`: `d6e85f3`** (receipt `dpl_4ztNuAtoYHkTz1tfnjz9Wph7ofQZ`, READY 2026-09-07T02:33Z) — the /ask redesign (field-first composer, a real running signal + Stop, 429 handling) and results opening the book at the quoted section with a return strip, Back healed (ADR-121). Branch `redesign/ask` = union of `fix/ux-overnight-sweep` (prior live) and `origin/main`, 22 commits ahead of main; the merge-to-main gap remains. `deploy.sh`'s identity check did not run (the CLI's READY poll timed out after READY); verified by hand from the Vercel API + alias, receipt annotated (WORKLOG 2026-09-06). Re-measure rather than read.
