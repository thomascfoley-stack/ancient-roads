...
Last verified: 2026-09-07 · **live on `ancientpaths.app`: `6dcd674f`** (receipt `dpl_2S3qFezZaS32CuWScxsiKcr3mwYc`, 2026-09-07T10:28:35Z, `alias_serves == deployment_id`, `state: live`) — **ADR-124**: one rule for rail row marks (the section glyph moves to the header; only My studies' per-study colour dot survives on a row), prayer rows lose their date stamps, and the pre-N4 custom sections are HIDDEN with their `localStorage` key deliberately untouched (the prayer carry-forward's only recovery source; possible teams/classroom feature later). Verified independently of the deploy script by fingerprinting the live JS chunks: four retired strings all absent, the new `pl-5` panel indent present, **positive controls (`rail-group-`, `Prayer journal`) present so the check is not vacuous**, negative control 0. `main` == live == `6dcd674f`; the code-diff predicate below is empty. Owner-executed — the agent session's permission classifier refuses `./deploy.sh`. Re-measure rather than read: `git diff --stat origin/main <live-sha> -- web/src src` empty means the gap is closed however far the shas differ.
