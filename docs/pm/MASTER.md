...
| 2 | Confirm `bge-large` **for user-corpus embedding** (the corpus is already pinned by ADR-005) | **STILL OPEN — and it is the one that bites next.** Slice 1 step 4 is "embed with the parity check", so this stops being deferrable there. The order rules "proceed on bge-large" and ADR-100 states explicitly that it does NOT close B2, so the code is being built on an assumption the owner has not ratified. Parity failure returns garbage silently, with no error: Jina v3 is ALSO 1024-dim, so a mismatch inserts, joins and scores cleanly |
