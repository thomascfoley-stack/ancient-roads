...
| 6 | **SEC-1 — the gate decision IS the public-launch decision.** The site password gate (`middleware.ts`, everything but `gate\|api/gate\|_next/\|favicon\|manifest\|icons`) stays up until the Neon Auth transitive CVEs are resolved. Note the second-order effect measured 2026-08-16: **nothing reaches `/api/ask` while the gate is up**, so `ask_outcomes` accumulates only from owner asks — Phase-D's ~1–2k training examples are blocked behind this decision, not merely "started". **BRIEFED 2026-08-16 — and the premise moved: [decision brief](orders/2026-08-16-sec1-decision-brief.md).** Six places in this repo state `@neondatabase/auth@0.4.2-beta` is the newest version that exists, hard-pinning `better-auth@1.4.18`. Re-measured (`npm view`): **`0.5.0-beta` exists and pins `better-auth@1.6.23`** — which patches BOTH declared reds (g38m ≥1.6.11, qq9h ≥1.6.22) and satisfies the standing "do not migrate below 1.6.11" rule for the first time. The bump resolves clean in scratch (single hoisted 1.6.23; residual advisories are dev tooling only) and has a two-file blast radius. **It does NOT close SEC-1 on its own** — `SECURITY.md:359-373` already records that the vulnerable auto-link runs on Neon's HOSTED server, so a client-SDK version is evidence about the platform, not proof of it; the question that settles it (`AUTH_CUTOVER_V2_NEON.md:131-135`) has been drafted and unsent since 2026-07-08. Four owner actions recommended. **BUMP EXECUTED 2026-08-16 on `work/02`, owner-approved in principle ([addendum §8](orders/2026-08-16-sec1-decision-brief.md)): API surface did NOT move — zero callers adapted — `tsc` 0/0, `next build` exit 0, and the browser wire exercised at desktop + 390px (no overflow) through `POST /api/auth/sign-in/email` to the SDK's outbound proxy call. NOT PROVEN: a real credential against Neon's hosted server — that needs the production `NEON_AUTH_BASE_URL` and an owner at the keyboard, and `neon-auth-live.test.ts` stayed loudly NOT RUN. **MERGE IS GATED ON DECISION 8 BELOW.** Gate untouched | public launch · Phase-D |
| 8 | **What mechanically records that SEC-1 is open, once the dependency tree is clean?** Filed 2026-08-16. The bump takes `better-auth` to 1.6.23, so both declared reds vanish and `npm run audit` fails correctly with `declared --expect-red id(s) no longer observed`. The obvious fix — emptying `--expect-red` — is a trap this repo already documented: `SECURITY.md:368-369` says that entry *"is currently the only mechanical record that SEC-1 is open."* After the bump **neither `ignoreGhsas` nor `--expect-red` carries an auth advisory**, so every check reads SEC-1 as CLOSED while the hosted-server question is unanswered. **Third instance in one day of the same shape — the remedy removes the marker, not the risk** (see also decision 7 and the §5 dead-`if`). SEC-1's true state lives on Neon's server where no local check can see it, so the honest replacement is a **dated attestation that fails the gate when stale**, not another advisory-list entry. Recommended, deliberately not implemented — it is an owner call about what the gate asserts | the SDK bump merging |
| 7 | **`MULTI_USER_UPLOADS` — the SEC-1 coupling is DORMANT, and the flag is `true`.** Filed 2026-08-16 with a red-proof (WORKLOG). `access.ts:19-21` promises `MULTI_USER_UPLOADS = true` goes RED while an in-path advisory sits in `ignoreGhsas`; measured, `stillIgnored` derives to `[]`, so the assertion at `sec1-upload-gate.test.ts:77-82` sits in a dead `if` and never executes — the test reports 7 passed having never evaluated its load-bearing claim. Cause: the 2026-08-11 relocation of g38m from `ignoreGhsas` to `--expect-red`, done for a good reason, emptied the condition. Seeded red (1 failed) and reverted green, so the guard is correct and its trigger is simply gone. **Watchlist, instance seventeen: a guard whose trigger was moved out from under it by an unrelated legitimate change.** Not fixed — re-arming it against the declared set turns CI red immediately, because the flag is `true`. Either the flag returns to `false`, or `true` is recorded as deliberate acceptance | multi-user upload · SEC-1 closure |
