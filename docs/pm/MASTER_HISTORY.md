# MASTER HISTORY — long-form narrative moved out of MASTER.md

Moved VERBATIM (no rewriting, no summarising, no dropping) from `docs/pm/MASTER.md` on
2026-08-23 under amendment A6 (docs/pm/orders/2026-08-22-autonomous-swarm-closeout.md §12A).
MASTER.md is the board; this file is the record behind the rows. Section anchors are
cited from MASTER.md as `MASTER_HISTORY.md §<anchor>`.

## §lane-a — Lane A gate rows (full history)
...
## §lane-b — Lane B gate rows (full history)
...
| B5 | Slice 1 — prose/sermons end-to-end + the tradition-gap join | **SHIPPED AND LIVE — row corrected 2026-08-21; it read IN PROGRESS for sixteen days after Slice 1 reached production (2026-08-05), still naming as *Next* the K-derivation its own B0a row records as DONE, and as *Gated* the tradition-gap join that has been deployed and serving (ADR-104 discharged: `LEGAL_CORPUS_FILTER` = `(served)`).** Note the 28-legs phrasing one clause back: 28 is the suite size; TWO legs were watched red (rls-two-account-REDPROOF.log). Current verified state lives in `docs/STATE_OF_TRUTH.md` §5 (rewritten 2026-08-21) and the deep-dive order `orders/2026-08-20-uploader-deep-dive.md`: measured 70% chapter-recall at the shipped K=3 on fresh held-out; the 2026-08-21 remediation wave closed H3/H4/H5/H8, D1/D2/D13-D19, M2 (ADR-100 detection BUILT — 69/69 family agreement, beat the KJV oracle, honest confidence) and M3 (digit-ordinal citation forms). Open from that order: H9's owner-terminal EXPLAIN, the asserted_ownership_at licensing column (owner), Slice 4 (/ask integration) with its RetrievalContext.traditions caveat, and the anchor backfill for pre-detection documents |

## §lane-c — Lane C, UX remediation (full history)
...
## §lane-d — Lane D, corpus CDN + /ask latency (full history incl. ingestion note + watchlist 15/16)
...
| D4 | B2 — where the seconds go in `/ask` | **DEV-LOCAL DONE 2026-08-15; both pre-registered rules UNTRIGGERED, so B4 built nothing.** p50 9.1s: retrieve 47.8% · compose 50.3% · verify ~0% · embed 1.8% · lanes 0ms (overlapped); 10/10 composed. Rule 1 50.4% (bar 60) · Rule 2 4.2s (bar 15). **The dev-local run says nothing about production — and the PROD RUN HAS SINCE HAPPENED; this row said it was still owed until 2026-08-21.** Owner-gated prod measurement the same day (post-deploy `e3b14cd`, n=25): **compose 74.4% · retrieve 23.8% · embed 1.7% · verify ~0%, p50 10.5s / p95 20.6s** — i.e. **Rule 1 FIRES at 74.5%**, the opposite of the dev-local verdict in the same row, and retrieve is FASTER on prod than dev (2.7s vs 4.2s p50). Both halves of Rule 1 were already shipped when it fired (`MAX_RETRIES=2`; source-streaming since `79be87c`, 2026-07-09), so the remaining lever is compose itself. Read the prod numbers as WARM: one local process against the prod DB, not 25 cold Lambdas — the dev-local cold start put 18.2s into retrieve on ask 1 against a 4.2s p50, so retrieve's share is understated for a genuinely cold invocation. [Dev-local evidence](../evidence/ask-latency/B2-measurement-2026-08-15-devlocal.md) · [prod n=25](../evidence/ask-latency/prod-25-measurement-2026-08-15.md) |
...
## §queued-a8 — Queued behind A8 (filed 2026-08-02)
...
## §ux-5 — the rail hid five features below an unmarked scroll (CLOSED 2026-08-07)
...
## §o-1 — ROTATE THE PRODUCTION DATABASE PASSWORD (full pre-flight + corrections)
...
## §lane-e — Lane E, corpus↔surface reconciliation (full history)
...
## §lane-f — Lane F, the gate that could not go green (full history incl. F4)
...
| F5 | ⚑ First green run | **ACHIEVED 2026-08-22 — run [`32562471249`](https://github.com/thomascfoley-stack/ancient-roads/actions/runs/32562471249) @ `2012e03`, BOTH jobs SUCCESS: the first green `db-invariants` in this repository's history.** The owner's criterion, ruled in chat 2026-08-22T06:31:36Z (session `699cded5`, quoted in ADR-119): *"F5 closes on the first run that's green with every suite truthfully accounted for — not on zero failing tests."* Measured on the run itself: 148 test files passed · 4 skipped ALL DECLARED (`secret-caused fully-skipped files: 0` → `skip ceiling OK`) · receipt `executed=1043 passed=1032 failed=0` → `receipt OK` · bible assets restored from cache in seconds (`Cache restored from key: bible-assets-f7dfe89bc5578afb`). What it took after F1–F3, all on 2026-08-22, each layer only visible once the previous one fell: migration 020 NOT VALID (replay-from-zero), the blob secret + the stale pipeline test, the bible-assets fetch (rate-limit backoff, save-on-complete cache), the skip-manifest race + the invariants-only path regex, the check-name/title mismatch, the single-record reader blind spot, and the second manifest reader single-sourced (`scripts/lib/skip-manifest.mjs`). WORKLOG 2026-08-22 carries every layer with verbatim evidence |
...
## §scanre — Queued: the SCAN_RE false-floor class
...
## §watchlist — the failure-mode watchlist (full)
...
unavailability, would make the signal mean one thing again. **CLOSED 2026-08-22 (W-VEC429):**
`web/test/helpers/provider-availability.ts` — bounded retry (≤3, exponential backoff with
jitter) on 429/5xx/network, persistent outage → loud-skip NOT RUN (`kind: 'provider'`), a
genuine failure (400/401, wrong vector) still fails. Wave 7 filed one LOW finding on its
message-regex classifier (verdicts/wave7-verdicts.md).
