...
**Status: REPAIRED ON THE DEV BRANCH (2026-07-16) for 6 of 14 authors; 8 quarantined; NOT promoted to
prod.** Barnes/Wesley/Calvin/Scofield/B.W. Johnson re-sourced per-verse from CrossWire SWORD modules
(`src/ingest/sword-zverse.ts`, DistributionLicense verified per `.conf`); **Poole re-sourced the same
night (v2) from the EEBO-TCP CC0 transcription** (`src/ingest/poole-tcp.ts`, 24,104 entries, 2.9%
collapse, staged); static corpus regenerated (`src/ingest/regen-crosswire-static.ts`);
`ingest-commentary-fts` re-run against dev — collapse 100% → 3.0–3.8%, forbidden provenance on these
authors → 0; `verse-keys.test.ts` un-skipped and green. The 8 authors with no clean per-verse source
(Cambridge/Pulpit/Benson/Bengel/MacLaren/Darby/Lange/Geneva — Geneva's CrossWire module carries NO
license grant, fail closed) are quarantined out of the static corpus to `data/quarantine/` (reversible;
the hold file also carries the old biblehub rows of the 6 re-sourced authors — 14 authors, 200,395 rows). Teacher OT: Calvin OT ingested (6,204 entries, the cleared
