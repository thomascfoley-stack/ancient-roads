...
| 6 | **~14% fallback rate** (≈1 in 7 answers shows raw sources, not a composed answer) | **MEDIUM** | **ACCEPTED — and EXPLICITLY UNMONITORED** (ADR-028) | Fail-closed-safe (never a wrong answer), and a degraded-but-honest response. Not a launch blocker, but a UX quality cost. ⚠️ **Corrected 2026-07-19:** this cell read "ACCEPTABLE (with monitoring)" — an unearned claim, and one this very table contradicts: **row 4 records that observability has no provider/DSN and is itself a BLOCKER**, so nothing is monitoring this. The rate is accepted as an **unmonitored** risk; wiring observability (`docs/OBSERVABILITY_DESIGN.md`) is a **stated prerequisite** before "with monitoring" may be claimed. The `ask_outcome` event makes it measurABLE, not measurED. **Reduce the dominant `schema` (invalid-block) compose failure** as a fast-follow. |
...
is the only deferral that is genuinely acceptable for launch — **accepted as an UNMONITORED risk**
(ADR-028). The earlier wording "acceptable for launch, with monitoring" was struck 2026-07-19: the
monitoring it leaned on is blocker 4 in this same table.
