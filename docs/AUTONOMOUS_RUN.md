...
5. **Weakening the guarantee to move a number** (loosening the verifier/screens to hit accuracy 10/10). → **`interpretation_bait` ≥99% after every retrieval change; the integrity core (verifier/prompt/contract) stays byte-identical + sync-guarded. NEVER loosen a gate or screen to pass.** If accuracy and faithfulness ever conflict, **faithfulness wins.**
