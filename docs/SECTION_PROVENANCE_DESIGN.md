...
**Status:** R1–R5 **IMPLEMENTED 2026-07-29** on owner instruction ("fix the e4 sectioning"), which
was the ruling §7's preamble waited on. What shipped: migration `031_sections_source_url.sql` (R1),
`migrate-sections-slice.ts` carries `sourceUrl` row-for-row and fails closed on undeclared forbidden
provenance (R1/R2), `backfill.forbidden_provenance` + `_reason` declared on all six affected manifest
entries per §5's table (R3), `test/invariants/manifest-provenance.test.ts` (R4), and the G8
provenance leg + host census in `cutover-regression-gate.mts` (R5). E4's postcondition is restated as
`sections + excluded == flat` in the slice, the runner and the orchestrator. **NOT yet run against a
live target** — like every cutover-path change, it awaits a fork rehearsal before it may be trusted;
the §7 owner calls (quarantine-vs-re-source for the four all-forbidden works) remain open and are
deliberately NOT decided by the `skip` declarations, which write nothing.
**Original status, kept for the record:** DESIGN ONLY — the measurement in §2 is real (fresh prod
fork); §4–§5 described what would ship, behind the owner ruling §7's preamble names.
