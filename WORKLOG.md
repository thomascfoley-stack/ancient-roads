...
| slice | proof |
|---|---|
| migration 120 `history_embeddings` (renumbered; a parallel session took 119 on prod) (dev, ledger `7f156545ea2d`) | its own DO tail went RED on first apply — dev default-privileges had silently granted app_runtime DML; the migration now REVOKEs explicitly |
...
`corpus-copy.mjs` dies on `!process.stdin.isTTY` with "a piped answer is not consent". The owner's
go authorises it; the gate still needs a person at the terminal. Command is in §6 of
[the treatment proposal](docs/pm/orders/2026-08-18-p4n-copy-treatment.md).

**Item 3 is done, and it turned out to be a code fix rather than the ruling it was filed as.**

The 2026-08-18 diagnosis §17.10 records that `commentary_entries` holds copyrighted and standing
MUST_NOT_SERVE material — Tyndale 15,161 · Origen 2,672 · CS Lewis · Chesterton · Wilson · Tolkien
— "gated only by not being named" on an 8-author allowlist, and asks for a ruling.

**Measured first: the repo already had the stronger mechanism and had not wired it here.**
`MUST_NOT_SERVE_AUTHORS` exists in `legal-corpus.ts` and is described at line 141 as "an absolute
veto"; `LEGAL_COMMENTARY_ENTRIES_PREDICATE` never referenced it. `commentary_entries` has no
`served` column and its `work` column is entirely NULL, so both mechanisms that make the VECTOR
surface unreachable are inert on the FTS surface — the allowlist was the only thing there.

So the veto is now ANDed into the predicate, mirroring `isMustNotServeAuthor` rule for rule (exact
hit, first-token normalisation, the `Jerome's …` bucket).

**Verified by execution against real rows on lane-b, not by reading:**
...
`fts-legal-index-sync.test.ts` went red: `LEGAL_COMMENTARY_ENTRIES_PREDICATE` is not only the query
filter, it **IS** the predicate of the partial index `idx_commentary_fts_legal`. Change the constant
without a rebuild migration and the planner silently stops using the index — the slow common-word
scan returns, which is the exact 2026-07-13 rot that guard was written for, and the same class as
MASTER.md's D1. So this is not the small licensing belt I first described: it is a code change
COUPLED to a prod DDL migration.

`db/migrations/117_fts_legal_must_not_serve_veto.sql` written to the 115 pattern: predicate
GENERATED BY PRINTING the constant (the 108 discipline, never hand-edited), zero-window rebuild
(CREATE CONCURRENTLY as `_v11` → DROP old → RENAME), run via `apply-migration-concurrent.mjs`.
**NOT APPLIED** — bylaw 7, owner-gated, and deliberately not applied by the session that wrote it.

**THE CODE AND THE MIGRATION MUST SHIP TOGETHER.** Deploying the code alone leaves results correct
and the FTS surface seq-scanning. That coupling is stated in the migration header too, because it is
the thing most likely to be forgotten.

**What this closes:** the three edits that would have undone the old arrangement — the A048
backfill filling `work` (which the diagnosis names as the trigger), adding an author to the
allowlist who also appears on copyrighted rows, or a new OR leg written without the veto in mind.
ANDed, the veto survives all three.

### NOT DONE / UNVERIFIED
- **The ruling is now NARROW but still the owner's.** `CS Lewis` (1,102), `GK Chesterton` (714),
  `Douglas Wilson` (16) and `JRR Tolkien` (11) **pass** the veto — they are not on
  `MUST_NOT_SERVE_AUTHORS`, so they remain excluded only by the weak mechanism. Adding names to
  that list is a content-quarantine call, which `AGENTS.md` reserves to the owner. Recommended:
  add all four; they are 20th-century copyrighted authors, it costs nothing today (they are not
  served), and it converts the last of this cohort to the strong mechanism.
- **One of my own test legs was wrong and is corrected in place.** It sliced from the veto to the
  end of the predicate and demanded no `) OR ` — which fails against a CORRECTLY ANDed veto,
  because the veto contains alternatives of its own. A regex over the tail cannot tell an internal
  OR from a readmitting one; the property lives in the operator that INTRODUCES the veto.
**A SECOND guard fired, and my first two attempts to fix it were both wrong.**
`ask-outcomes-migration.test.ts` asserted `Math.max(...nums) === 116` — "nothing higher" — which is
true only until the next migration exists. 117 broke it, correctly authored and applied to nothing.
A check that fails on correct work teaches people to edit checks, so it had to change; the question
was to what.

- Attempt 1: replace it with "no gaps". **Also wrong** — measured, the sequence legitimately jumps
  at **14, 44 and 100**, because the 100-series is Lane B's separate range (100/101/102 exist only
  on `lane-b-uploader`, MASTER.md B1). That would have redded a perfectly correct tree, i.e. the
  same defect I was removing.
- Attempt 2, shipped: assert only what is true and durable — **the number is not reused.** Ordering
  is enforced by the ledger and `apply-pending.mjs`, not by a filename. Red-proofed with a
  duplicate 116.
