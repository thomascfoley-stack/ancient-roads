...
>
> **BARNES FIRST SLICE — DONE + PROVEN GREEN (2026-07-10).** Migration `006` applied (sources/sections/section_anchors/section_embeddings). Barnes re-pointed: **1,300 embeddings → 1,300 sections = 1,300 anchors = 1,300 section_embeddings** (reused vectors, $0). **Gate A (sections) = 0**, **Gate B PASSED** (published Barnes + config manifest), **`npm run audit` green**. Legacy retrieval untouched (dual-read). **Stopped before the other ~400 sources.** Next unit (on approval): the section-based retrieval bridge + diagnostic parity, then scale the backfill to all 401 works.

- **KNOWN LIMITATION (tracked — tied to eval-set growth):** the legacy `source_id` has no `entry_index`, collapsing **341,912 eligible entries → 168,233 keys** — only the first entry per key is embedded, so **~173,679 entries are FTS-searchable but not in the vector index**. Path A preserves this as-is (fixing it = re-embedding = cost + a reopened coverage gap). Whether the collapse actually caps accuracy is **unknown until the eval set is broadened** (NEXT_PHASE §4, currently Gospel/reformed-heavy). **Revisit corpus expansion once the larger eval can measure it — decide with data.** Section identity is surrogate + append-only (`MIGRATION_DESIGN.md` §4.1) so expansion is a pure insert, never a re-migration. *(Also tracked: corpus `sourceUrl` = biblehub, an ADR-008-forbidden aggregator — text is PD so license is valid, but re-source from CrossWire/PD before wide rollout.)*
