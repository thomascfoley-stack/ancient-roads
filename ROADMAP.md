...
> - **Patristic classify RUN (read-only) — whole-corpus crawl CONTAMINATED, no false number reported.** New Advent BFS crawl reached only 2,004/~5,000 pages unevenly (Chrysostom content missed → false quarantines) + New Advent lacks the Catena Aurea. Raw 12.6% repairable / 82% quarantine is a *floor/inflated-ceiling*, not the truth. Reliable: crawled ANF/NPNF core (Augustine, Tertullian, Clement, Cyprian) repairs at 77–95%. **True number needs a complete multi-source PD corpus (full New Advent + Catena-Newman + Gutenberg) or per-work targeting, then re-run.** (`RESOURCING_PLAN` §11.)
> - **Still to clear before publish:** the true patristic classify (complete corpus) + libsword CrossWire-5. Then the final corpus-shape tally (published/quarantined/dropped) before publish.
