...
> - **Recommended next:** build **verse-ref intent routing** (reference-parse → target passage) — the real bottleneck; re-measure; add Catena if Gospel <2-voices persist. Then the launch-readiness accuracy on the legal corpus. Patristic tail + CrossWire-5 dropped as low/zero ROI.
