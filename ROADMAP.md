...
| **Highlights + Notes (annotations)** | Partial | `web/src/lib/annotations.ts` (`runAsUser`, 8 fns; `upsertNote` now atomic `INSERT … ON CONFLICT` on the unique partial index, migration `002`), `api/annotations{,/all}/route.ts`, `app/library/notes/page.tsx`; all 8 fns pass staging on `app_runtime` (RLS on) | Built + tested + **RLS enforced in prod** + in audit | RLS **not live in prod** (SEC-2 flip pending); no web tests; outside `/audit` | **[SEC-2](docs/SECURITY.md)** | User data · P0 |
| **SEC-2 — least-priv role + RLS enforcement** | Partial | `web/src/lib/db.ts` `runAsUser`; migrations `001` (role+grants+RLS) + `002` (unique note index); **staged on branch `sec2-stage`: real repo layer × 14 checks pass on `app_runtime` (pooled, RLS on)** incl. upsert insert+update, cross-user isolation, backstop; `docs/SECURITY.md` | `app_runtime` + RLS live in prod, verified via repo layer | **Staging verified**; only the prod flip remains (create role+grants, set `APP_DATABASE_URL`, redeploy) — held for go | user go-ahead; owner pw rotation (SEC-3) | Security · P0 |
