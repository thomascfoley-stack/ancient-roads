...
> - **Reference/pericope routing — BUILT + PROVEN (ADR-015, `REFERENCE_ROUTING_DESIGN.md`).** Soft-boost injection of on-range voices + an **on-passage floor** (top-2 reserved slots). Re-measure on the frozen 88: **legal verse-ref HIT=1 46%→96%, legal ≥2-voices 85%→91%, no full-corpus regression** (full ≥2-voices 98%). Soft-boost alone was insufficient (46%→50%) — the floor is the lever (the reranker owns final order). Retrieval-only; verifier/contract untouched. Migration 007 applied; production `retrieveCommentary` wired; audit green. `no-content` still 0.
> - **Recommended next:** (1) greenlight the **deploy** of this slice — meets the gate, nothing else changed. (2) Decide the residual **topical** <2-voices (5/88, e.g. "propitiation") → wire **Catena Aurea** (Gospels, no install) or accept as beta breadth. Then launch-readiness accuracy on the legal corpus. libsword/CrossWire-5 + patristic tail stay dropped (no-content=0).
