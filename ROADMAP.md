...
## Update 2026-07-16 (corpus ingestion v1+v2 — branch `ingest`, DEV only, staged)

`pnpm gate:ingest` wired over the existing gate modules (license/provenance/must-not-serve irreversible-first;
verse-keys, count-parity, content-sanity, text-match, versification, coverage). Verse-key collapse REPAIRED on
...
|---|---|---|---|---|---|---|
| **Study Docs P1 — data core (migration 110, /api/studies/\*\*, clipping engine, servability, export, purge)** | Partial | Branch `feat/study-docs-p1`; `db/migrations/110_studies.sql` applied+red-proofed on dev; `web/src/lib/{studies,servability,study-export}.ts`; `scripts/study-clipping-purge.mjs`; suites `web/test/{study-position,study-export}.test.ts`, `web/test/regression/studies-routes.test.ts`; 45/45 hand-test + logs in `docs/evidence/study-docs-p1/`; `npm run audit` green (WORKLOG 2026-08-12) | STUDY_DOCS_BUILD §4.1 met by the builder; **§4.3 (ship) needs P2 UI + S-1…S-14 and P3 independent re-verification** | UI + invariant suite (P2 swarm); independent verify + browser passes + deploy (P3); RLS-under-Neon-id (C5) still unproven; provenance-refusal corpus case not exercisable on dev | P2/P3 sessions; owner gate for any prod contact | Feature·Studies · P1 |
...
| **Eval harness + interpretation-bait suite** | Partial | `src/evals/{run,checks,types}.ts`; `test/evals.test.ts` (5) pass; `evals/cases/{interpretation_bait(35),format,diversity,refusal_shape}.yaml`. **interpretation_bait EXECUTED live through the real `teach()` 2026-07-10 → 35/35 = 100%, 0 breaches reached the user** (WORKLOG; the fail-closed verifier was observed catching prescription + fabrication wobbles) | Harness (Done-level) **and** suites executed vs a teacher, bait ≥99% (needs ~300 clean cases) | bait clean at n=35 (35/35, 0 breaches = **~92% lower bound** — NOT "≥99% MET"; ≥99% needs ~300 clean cases, not a re-run of 35). Remaining: grow the set to ~300 real queries, THEN the ≥99% claim is earnable; CI-wire it via a permanent **authed** harness (the 2026-07-10 run used a throwaway local endpoint) | AI generation (present) | Intelligence · P0 |
