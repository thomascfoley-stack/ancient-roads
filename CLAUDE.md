...
2. **Accuracy** — the retrieved sources are the *right* ones ("good shepherd" → John 10, not Luke 2). Measured by the frozen held-out eval (v3), **per category.** Last (2026-07-13): verse-ref HIT@1 95 / HIT@2 98 · pericope 87 / 100 · proper-noun 70 / 90 · **epistle HIT@2 84** (n=25) · **topical HIT@2 75 frozen → 80 relabeled** (n=20; §1b showed the frozen topical labels are under-specified) · control clean. **These n are too small to call either category "below 85": topical's 95% CI is [53, 89] and epistle's is [65, 94] — 85 sits INSIDE both.** So the 85/85 gate is currently **unmeasurable, not failed** — the honest next step is a larger v4 (n≈100 per stratum) or a gate that needs no label at all (≥2 distinct grounded voices per answer, the actual product promise); see WORKLOG §7. Re-run the eval on every retrieval change and record it in `WORKLOG.md`.
