...
2. **Accuracy** — the retrieved sources are the *right* ones ("good shepherd" → John 10, not Luke 2). Measured per category on frozen held-out sets. Current (2026-07-18, the option-(c) lane config — exegetical pool = verse-commentary + fathers; sermons/theology in labeled lanes; WORKLOG 2026-07-18): **honest v3 baseline** (v3 is now a **dev set** — measured against repeatedly, never gate on it) verse-ref 95/95 · pericope 87/100 · epistle 68/80 · topical 45/75 · proper-noun 60/90 · controls clean; **frozen v4** (hash `90de5dc3`, bars pre-registered before the run, run ONCE, no tuning) verse-ref 100/100 · pericope 80/100 · epistle 96/100 · topical 80/90 · proper-noun 60/100 · controls clean · no-content 0/110 — **cleared every pre-registered bar except proper-noun on that July run — and the accuracy status is NOT restated here.** **[ADR-028 + its amendment ADR-116](docs/DECISIONS.md) are the only place it is stated. Go read them; do not re-derive it from this file.** In one line so you know whether you need to: the proper-noun gate is **HIT@2** (not HIT@1), the July miss **closed on 2026-08-02**, and the ADR-118 proper-noun gate **FIRED OPEN on its first measurement (2026-08-22): 17/20 = 85% against the 18/20 bar** — labels are being re-coded before any recount (both wrong-passage misses returned 0 voices, a no-content shape; see ADR-118 and `docs/pm/orders/2026-08-22-swarm-recovery-amendment.md`). Read every number in this paragraph with its denominator — proper-noun is **n=10**, so a one-query swing moves it 10 points. (This sentence has now gone stale twice in one day, in opposite directions — advertising a ruled decision as open, then quoting a superseded figure as current. It is a pointer now for that reason.) **The small-n caution still applies:** topical 90 and pericope 80 are point estimates whose 95% CIs straddle their bars — "clears" means point-estimate-clears, not proven-above; v4's KJV-phrase-anchored labels make the doctrinal strata easier than v3's abstract queries (the abstract-topical failure mode is not exercised); and v4 samples no Song of Solomon, so no-content 0/110 does not clear the known SoS hole. See `docs/HELDOUT_EVAL_DESIGN.md` §v4 + WORKLOG 2026-07-18. Re-run the eval on every retrieval change and record it in `WORKLOG.md`.
