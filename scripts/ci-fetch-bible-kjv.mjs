// CI corpus assets — fetch web/public/bible/kjv from the public corpus blob store.
...
// web/public/bible/kjv, gitignored (.gitignore:22) and therefore absent from every CI checkout.
// `availableTranslations()` derives from disk, so kjv alone is sufficient: detection runs
// against one translation and the pipeline completes.
//
...
const TARGET = path.join(ROOT, 'web/public/bible/kjv');
const PREFIX = 'bible/kjv/';
...
if (process.argv.includes('--print-cache-key')) {
  const digest = createHash('sha256')
    ...
    .digest('hex')
    .slice(0, 16);
  process.stdout.write(`bible-kjv-${digest}\n`);
  process.exit(0);
}

if (files.length === 0) {
  console.log(`::warning title=bible/kjv fetch::manifest lists no ${PREFIX} entries — nothing fetched, suites will skip honestly`);
  process.exit(0);
}
...
if (existsSync(TARGET) && files.every(present)) {
  console.log(`bible/kjv already complete: ${files.length} files (cache hit or operator tree)`);
  process.exit(0);
}

const started = Date.now();
const tmp = path.join(ROOT, `web/public/bible/.kjv-fetch-tmp-${process.pid}`);
...
if (failed) {
  rmSync(tmp, { recursive: true, force: true });
  console.log(`::warning title=bible/kjv fetch FAILED (suites will skip honestly)::${failed.message} — ${done}/${files.length} fetched, nothing installed`);
  process.exit(0);
}
...
console.log(`bible/kjv installed: ${files.length} files in ${((Date.now() - started) / 1000).toFixed(1)}s from ${base}`);
