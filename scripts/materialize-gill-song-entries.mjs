// Materialize gill-song into `commentary_entries` so Song of Songs stops being the one book of 66
// that passage search cannot see.
//
...
try {
  ...
  const { rows: prov } = await client.query(
    `SELECT src.license, count(*) FILTER (WHERE s.source_url ~* '(biblehub|studylight|historicalchristian)') AS dirty
       FROM sources src JOIN sections s ON s.source_id = src.id WHERE src.slug='gill-song' GROUP BY src.license`);
  ...
  const { rows: conv } = await client.query(
    `SELECT mode() WITHIN GROUP (ORDER BY year) AS year, mode() WITHIN GROUP (ORDER BY tradition) AS tradition,
            min(source_url) AS source_url FROM commentary_entries WHERE author='John Gill'`);
  const { year, tradition, source_url } = conv[0];
  const { rows: title } = await client.query(`SELECT title FROM sources WHERE slug='gill-song'`);

  const { rows: existing } = await client.query(
    `SELECT count(*)::int n FROM commentary_entries WHERE author='John Gill' AND book=22`);
  ...
  const ins = await client.query(
    `INSERT INTO commentary_entries (book, chapter, verse_start, verse_end, author, year, tradition,
     ...
     SELECT (sa.verse_id_start/1000000)::smallint,
      ...
      WHERE src.slug = 'gill-song' AND sa.verse_id_end % 1000 <> 999
      RETURNING id`,
    [year, tradition, title[0].title, source_url]);
  ...
  const { rows: dup } = await client.query(
    `SELECT count(*)::int n FROM (SELECT 1 FROM commentary_entries WHERE book=22
  ...
  const { rows: bad } = await client.query(
    `SELECT count(*)::int n FROM commentary_entries WHERE book=22 AND author='John Gill'
      AND (verse_end >= 900 OR length(body) > ${BODY_CAP})`);
  if (bad[0].n > 0) throw new Error(`REFUSING: ${bad[0].n} row(s) violate the table's own conventions`);

  const ids = ins.rows.map((r) => r.id);
  writeFileSync(`docs/evidence/song-of-songs-materialization-ids.json`,
    JSON.stringify({ slug: 'gill-song', author: 'John Gill', book: 22, inserted: ids.length, ids }, null, 2));
  console.log(`  inserted ${ids.length} row(s); skipped the 8 chapter-level sections by design`);
  console.log(`  INVERSE: DELETE FROM commentary_entries WHERE author='John Gill' AND book=22;`);
